...
if ( ! function_exists( 'understrap_bootstrap_comment_form_fields' ) ) {
	...
	function understrap_bootstrap_comment_form_fields( $fields ) {
		...
		$fields    = array(
			'author'  => '<div class="form-row"><div class="form-group comment-form-author col-md-4"><label for="author">' . __( 'Name',
					'understrap' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
			            '<input class="form-control" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . '></div>',
			'email'   => '<div class="form-group comment-form-email col-md-4"><label for="email">' . __( 'Email',
					'understrap' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
			            '<input class="form-control" id="email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . '></div>',
			'url'     => '<div class="form-group comment-form-url col-md-4"><label for="url">' . __( 'Website',
					'understrap' ) . '</label> ' .
